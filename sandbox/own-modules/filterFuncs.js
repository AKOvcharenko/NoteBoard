...
const filterFunctions = {
    ...
    [constants.CONTENT_FILTER](arr, value){return arr.filter(el =>
        this.isEmpty(el, ['title', 'text']) || `${el.title} ${el.text}`.indexOf(value) > -1)},

    [constants.PRIORITY_FILTER](arr, value){return arr.filter(el =>
        this.isEmpty(el, ['priority']) || el.priority.indexOf(value) > -1)},

    [constants.DATE_FROM_FILTER](arr, value){return arr.filter(el =>
        !this.isNeededDateFormat(value) || new Date(el.creationDate) > new Date(value))},

    [constants.DATE_TO_FILTER](arr, value){return arr.filter(el =>
    !this.isNeededDateFormat(value) || new Date(el.creationDate) < new Date(value))}
};
